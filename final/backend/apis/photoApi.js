...
const createPhoto = async (req, res) => {
    ...
    try {
        ...
        const { isSuspicious, extractedText } = await checkForWatermark(photo_url);

        if (isSuspicious) {
            console.warn('Watermark or reuse suspected. Extracted text:', extractedText);
            return res.status(400).json({
                error: "Image contains watermark or suspected reused content.",
                extractedText
            });
        }
...
// GET /admin/photos
const getAllPhotosForAdmin = async (req, res) => {
    try {
        const photos = await Photo.find();
        res.status(200).json(photos);
    } catch (error) {
